...
@Slf4j
@OwnedBy(PL)
public class AddParentUniqueIdMigrationProject implements NGMigration {
  public static final int BATCH_SIZE = 500;
  public static final String PROJECT_PARENT_UNIQUE_ID_GENERATION_LOG_CONST =
      "[NGUpdateParentUniqueIdProjectMigration]:";
  private final MongoTemplate mongoTemplate;
  @Inject
  public AddParentUniqueIdMigrationProject(MongoTemplate mongoTemplate) {
  ...
  public void migrate() {
    log.info(format(
        "%s Starting migration for Entity Type: [%s] ", PROJECT_PARENT_UNIQUE_ID_GENERATION_LOG_CONST, "Project"));
    ...
    log.info(format("%s Entity Type: [%s], Total count: [%s]", PROJECT_PARENT_UNIQUE_ID_GENERATION_LOG_CONST, "Project",
    ...
    try {
      for (Iterator<Project> iterator =
        ...
        if (nextProject != null && isNotEmpty(nextProject.getParentId()) && isEmpty(nextProject.getParentUniqueId())) {
          ...
          Update update = new Update().set(ProjectKeys.parentUniqueId, nextProject.getParentId());
      ...
      log.error(
          format("%s job failed for Entity Type: [%s]", PROJECT_PARENT_UNIQUE_ID_GENERATION_LOG_CONST, "Project"), exc);
    }
    log.info(format(
        "%s Migration for Entity Type: [%s]. Total documents: [%d], documents to Update: [%s], Successful: [%d], Failed: [%d]",
        PROJECT_PARENT_UNIQUE_ID_GENERATION_LOG_CONST, "Project", totalCounter, updateCounter, migratedCounter,
