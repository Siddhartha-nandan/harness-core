...
public class ScopeNameMapper {
  ...
  public ScopeNameDTO toScopeNameDTO(@NotNull ScopeDTO scopeDTO) {
    ...
    if (!isBlank(scopeDTO.getOrgIdentifier())) {
      orgName = organizationService.get(ScopeInfo.builder().accountIdentifier(scopeDTO.getAccountIdentifier()).scopeType(ScopeLevel.ACCOUNT).uniqueId(scopeDTO.getAccountIdentifier()).build(), scopeDTO.getOrgIdentifier())
