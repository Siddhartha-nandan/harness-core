...
public class CEReportScheduleServiceImpl implements CEReportScheduleService {
  ...
  public List<CEReportSchedule> update(String accountId, CEReportSchedule schedule) {
    ...
    CronExpression cronSequenceGenerator = CronExpression.parse(schedule.getUserCron());
    Date next = Date.from(cronSequenceGenerator.next(LocalDateTime.now()).toInstant(ZoneOffset.of(cronTimeZone)));
