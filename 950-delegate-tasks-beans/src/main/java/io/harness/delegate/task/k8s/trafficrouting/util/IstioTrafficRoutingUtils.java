...
public class IstioTrafficRoutingUtils extends TrafficRoutingUtils {
  @Override
  public int sumDestinationsWeights(List destinations) {
    return sumDestinationWeights(
        destinations, d -> d != null && d.getWeight() != null, HttpRouteDestination::getWeight);
  }
  ...
  public List<Triple<String, Integer, Integer>> getDestinationsNormalizationInfo(List destinations, int cap) {
    ...
    ((List<HttpRouteDestination>) destinations).stream().forEach(dest -> {
      int normalizedWeight = normalizeWeight(sum, dest.getWeight(), destinations.size(), cap);
      normalizedDestinations.add(Triple.of(dest.getDestination().getHost(), dest.getWeight(), normalizedWeight));
    ...
    return normalizedCorrectedDestinations.stream()
        .map(dest
            -> HttpRouteDestination.builder()
                   .destination(Destination.builder().host(dest.getLeft()).build())
                   .weight(dest.getRight())
                   .build())
