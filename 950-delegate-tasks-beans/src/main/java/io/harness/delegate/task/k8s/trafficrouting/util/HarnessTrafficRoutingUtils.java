...
public class HarnessTrafficRoutingUtils extends TrafficRoutingUtils {
  @Override
  public int sumDestinationsWeights(List destinations) {
    return sumDestinationWeights(
        destinations, d -> d != null && d.getWeight() != null, TrafficRoutingDestination::getWeight);
  }

  public List<Triple<String, Integer, Integer>> getTrafficRoutingDestinationNormalizationInfo(
      List<TrafficRoutingDestination> destinations) {
    return getDestinationsNormalizationInfo(destinations, 100);
  }
  ...
  public List<Triple<String, Integer, Integer>> getDestinationsNormalizationInfo(List destinations, int cap) {
    ...
    ((List<TrafficRoutingDestination>) destinations).stream().forEach(dest -> {
