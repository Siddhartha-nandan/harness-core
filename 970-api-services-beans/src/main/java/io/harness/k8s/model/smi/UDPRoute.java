...
import static io.harness.k8s.model.smi.Constants.UDP_ROUTE;
...
public class UDPRoute extends SMIRoute {
  ...
  public UDPRoute(String apiVersion, Metadata metadata, RouteSpec spec) {
    super(apiVersion, UDP_ROUTE, metadata, spec);
    validate();
  }
  void validate() {
    if (spec.getMatches().stream().noneMatch(PortMatch.class ::isInstance)) {
      throw new IllegalArgumentException("Unsupported spec in the UDPRoute");
    }
  }
}
