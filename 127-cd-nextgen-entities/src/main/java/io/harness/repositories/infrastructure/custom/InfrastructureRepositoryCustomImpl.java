...
public class InfrastructureRepositoryCustomImpl implements InfrastructureRepositoryCustom {
  ...
  public InfrastructureEntity moveInfrastructure(
    ...
    if (INLINE_TO_REMOTE.equals(moveConfigOperationDTO.getMoveConfigOperationType())) {
      InfrastructureEntity infraToUpdate =
          infrastructure.withRepo(moveConfigOperationDTO.getRepoName())
              ...
              .withRepoURL(gitAwareEntityHelper.getRepoUrl(infrastructure.getAccountIdentifier(),
                  infrastructure.getOrgIdentifier(), infrastructure.getProjectIdentifier()))
              .withFallBackBranch(moveConfigOperationDTO.getBranch());

      return moveToRemote(criteria, infraToUpdate);
    } else {
