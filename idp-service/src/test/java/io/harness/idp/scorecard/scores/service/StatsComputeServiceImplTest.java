...
public class StatsComputeServiceImplTest extends CategoryTest {
  ...
  @Captor private ArgumentCaptor<List<CheckStatusEntity>> checkStatusCaptor;
  ...
  public void testPopulateStatsData() {
    ...
    when(scorecardStatsRepository.saveOrUpdate(any(), any())).thenReturn(ScorecardStatsEntity.builder().build());
    mockCheckStatsSaveOrUpdate();
    statsComputeServiceImpl.populateStatsData();
    verify(scorecardStatsRepository, times(4)).saveOrUpdate(any(), any());
    verify(checkStatsRepository, times(10)).saveOrUpdate(any(), any(), any(), any());
    verify(checkStatusRepository).saveAll(checkStatusCaptor.capture());
    List<CheckStatusEntity> checkStatusEntities = checkStatusCaptor.getValue();
    assertEquals(5, checkStatusEntities.size());
  }
  ...
  private void mockCheckStatsSaveOrUpdate() {
    when(checkStatsRepository.saveOrUpdate(any(), any(), anyString(), anyString()))
