...
if [[ "" != "$STO_MANAGER_BASE_URL" ]]; then
  export STO_MANAGER_BASE_URL; yq -i '.yamlSchemaClientConfig.yamlSchemaHttpClientMap.sto.serviceHttpClientConfig.baseUrl=env(STO_MANAGER_BASE_URL)' $CONFIG_FILE
fi

if [[ "" != "$STO_MANAGER_SERVICE_CONNECT_TIMEOUT_IN_SECONDS" ]]; then
  export STO_MANAGER_SERVICE_CONNECT_TIMEOUT_IN_SECONDS; yq -i '.yamlSchemaClientConfig.yamlSchemaHttpClientMap.sto.serviceHttpClientConfig.connectTimeOutSeconds=env(STO_MANAGER_SERVICE_CONNECT_TIMEOUT_IN_SECONDS)' $CONFIG_FILE
fi

if [[ "" != "$STO_MANAGER_SERVICE_READ_TIMEOUT_IN_SECONDS" ]]; then
  export STO_MANAGER_SERVICE_READ_TIMEOUT_IN_SECONDS; yq -i '.yamlSchemaClientConfig.yamlSchemaHttpClientMap.sto.serviceHttpClientConfig.readTimeOutSeconds=env(STO_MANAGER_SERVICE_READ_TIMEOUT_IN_SECONDS)' $CONFIG_FILE
fi
...
replace_key_value gitSdkConfiguration.gitManagerGrpcClientConfig.target "$INTERNAL_NG_MANAGER_GITSYNC_TARGET"
replace_key_value gitSdkConfiguration.gitManagerGrpcClientConfig.authority "$INTERNAL_NG_MANAGER_GITSYNC_AUTHORITY"
replace_key_value grpcClientConfigs.cv.target "$INTERNAL_CVNG_MANAGER_TARGET"
replace_key_value grpcClientConfigs.cv.authority "$INTERNAL_CVNG_MANAGER_AUTHORITY"
replace_key_value grpcClientConfigs.srm.target "$INTERNAL_CVNG_MANAGER_TARGET"
replace_key_value grpcClientConfigs.srm.authority "$INTERNAL_CVNG_MANAGER_AUTHORITY"
replace_key_value grpcClientConfigs.ci.target "$INTERNAL_CI_MANAGER_TARGET"
replace_key_value grpcClientConfigs.ci.authority "$INTERNAL_CI_MANAGER_AUTHORITY"
replace_key_value grpcClientConfigs.sto.target "$INTERNAL_STO_MANAGER_TARGET"
replace_key_value grpcClientConfigs.sto.authority "$INTERNAL_STO_MANAGER_AUTHORITY"
replace_key_value grpcClientConfigs.iacm.target "$INTERNAL_IACM_MANAGER_TARGET"
replace_key_value grpcClientConfigs.iacm.authority "$INTERNAL_IACM_MANAGER_AUTHORITY"
replace_key_value grpcClientConfigs.idp.target "$INTERNAL_IDP_SERVICE_TARGET"
replace_key_value grpcClientConfigs.idp.authority "$INTERNAL_IDP_SERVICE_AUTHORITY"