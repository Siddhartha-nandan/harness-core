...
CONFIG_FILE=/opt/harness/iacm-manager-config.yml
...
if [[ "" != "$IACMMANAGER_MONGO_URI" ]]; then
  export IACMMANAGER_MONGO_URI; yq -i '.iacmmanager-mongo.uri=env(IACMMANAGER_MONGO_URI)' $CONFIG_FILE
fi
...
if [[ "" != "$IACM_TERRAFORM_IMAGE" ]]; then
  export IACM_TERRAFORM_IMAGE; yq -i '.ciExecutionServiceConfig.stepConfig.iacmTerraform.image=env(IACM_TERRAFORM_IMAGE)' $CONFIG_FILE
fi
